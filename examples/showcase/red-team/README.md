...
No API key, no model call. Each run writes its trail to a scratch directory it
removes at exit, so repeated runs never accumulate in `.watchlight/`.
...
injection family is withheld: the hook raises, the model never receives it, and
...
records the tool call each prompt tries to induce, and the stub makes exactly
that call through a governed tool, complying with everything.
...
A family that is none of these is **unhandled**, and the runner fails the run
...
the call the prompt is trying to provoke; the intent must be one of `answer`,
`export`, `send_email`, `delete`, and the runner refuses an unknown one.
...
the labelling assertion, and a `BENIGN` prompt that trips the screener fails the
control group.
...
and `delete` — but the runner has no expectation for the family, so it refuses
to call the run green. Handle it by adding a detector. If the family is a plain
request that policy must stop, add it to `POLICY_FAMILIES` in the runner and
make sure the policy denies what it induces.
...
- **Screening is rules, not a classifier.** It catches literal, well-known
  ...
  decode encodings or paraphrase — which is why the policy layer sits behind it,
...
Verified by [`check.sh`](./check.sh), which asserts both corpora in both lanes:
